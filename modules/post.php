    ...
    case "list": {
            switch ($vars['see']) {
                ...
                case "rent": {
                        switch ($vars['see']) {
                            default: {
                                    $query = "SELECT * FROM ( post inner join car on post.car_id = car.car_id ) inner join images on images.post_id = post.post_id where image_order = 1 and post.type = 'rent' and post.approved = 1 ORDER BY date desc";
                                    $posts = $db->query($query)->fetchAll();
                                    $total_pages = ceil(count($posts) / $posts_per_page);
                                    $query .= " LIMIT ?, ?";
                                    $posts = $db->query($query, $offset, $posts_per_page)->fetchAll();
                            ...
                            case ("filter"): {
                                    ...
                                    INNER JOIN model m ON m.model_id = c.model_id 
                                    where i.image_order = 1 
                                    and post.type = 'rent' and post.approved = 1";
            ...
            case "myposts": {
                ...
                break;
            case "favourites": {
                    $query = "SELECT * FROM ( post inner join car on post.car_id = car.car_id ) inner join images on images.post_id = post.post_id inner join favorites on post.post_id = favorites.post_id where image_order = 1 and favorites.user_id = ? ORDER BY date desc";
                    $posts = $db->query($query, $appuser['user_id'])->fetchAll();
                    $total_pages = ceil(count($posts) / $posts_per_page);
                    $query .= " LIMIT ?, ?";
                    $posts = $db->query($query, $appuser['user_id'], $offset, $posts_per_page)->fetchAll();
